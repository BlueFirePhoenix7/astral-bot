...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!args[0]) {
            ...
            // get commands in Admin category
            const adminCommandsList = [];
            readdirSync(`./commands/Admin`).forEach((file) => {
                const filen = require(`../../commands/Admin/${file}`);
                const name = `\`${filen.name}\``
                adminCommandsList.push(name);
            });
            ...
            const helpEmbed = new client.discord.MessageEmbed()
                ...
                .addField("🔧 - Admin Commands", adminCommandsList.map((data) => `${data}`).join(", "), true)
            ...
            if (!command) {
                ...
                let permission = command.permission || "No permission provided!"
